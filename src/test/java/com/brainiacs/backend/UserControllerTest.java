...
class UserControllerTest {
  ...
  void getAll_shouldReturn200WithEmptyPage() throws Exception {
    ...
    when(userService.getUsers(1, 10)).thenReturn(emptyPage);
  ...
  void getAll_shouldReturn200WithUsers() throws Exception {
    ...
    when(userService.getUsers(1, 10)).thenReturn(page);
  ...
  @Test
  void getUsers_shouldReturn400_whenPageIsLessThanOne() throws Exception {
    mockMvc.perform(get("/api/users").param("page", "0")).andExpect(status().isBadRequest());
  }

  @Test
  void getUsers_shouldReturn400_whenSPageizeExceedsMax() throws Exception {
